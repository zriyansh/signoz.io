...
```bash
OTEL_SERVICE_NAME='service name' OTEL_EXPORTER_JAEGER_ENDPOINT="http://ingest.signoz.io:14268/api/traces" opentelemetry-instrument -e jaeger python program.py
```
...
```bash
OTEL_SERVICE_NAME=<service name> OTEL_EXPORTER_JAEGER_ENDPOINT="http://ingest.signoz.io:14268/api/traces" LS_ACCESS_TOKEN=<access_token> opentelemetry-instrument -e jaeger <your run command>
```
